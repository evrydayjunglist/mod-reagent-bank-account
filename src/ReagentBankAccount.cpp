...
uint32 g_maxOptionsPerPage = DEFAULT_MAX_OPTIONS;

// AzerothCore module: Account-wide Reagent Bank
// This script adds a reagent bank NPC that allows players to deposit and withdraw reagents account-wide.
...
private:
    ...
    std::string GetCachedItemIcon(uint32 entry, uint32 width, uint32 height, int x, int y) const
    {
    ...
    // Returns a colored item link string for display in gossip menus (no cache, as it may be locale-dependent)
    ...
    // Withdraws a stack or all of a reagent from the account-wide bank for the player
    void WithdrawItem(Player *player, uint32 entry)
    {
        QueryResult result = CharacterDatabase.Query(
            "SELECT amount FROM mod_reagent_bank_account WHERE account_id = %u AND item_entry = %u",
            player->GetSession()->GetAccountId(), entry);
        if (result)
        {
            ...
            if (!temp)
            {
                ChatHandler(player->GetSession()).PSendSysMessage("Error: Item template not found for entry %u.", entry);
                return;
            }
            ...
            if (storedAmount <= stackSize)
            {
                ...
                if (msg == EQUIP_ERR_OK)
                {
                    ...
                    ChatHandler(player->GetSession()).PSendSysMessage("Withdrew %u x %s.", storedAmount, temp->Name1.c_str());
                }
                else
                {
                    player->SendEquipError(msg, nullptr, nullptr, entry);
                    ChatHandler(player->GetSession()).PSendSysMessage("Not enough bag space to withdraw %u x %s.", storedAmount, temp->Name1.c_str());
                    return;
                }
            }
            else
            {
                ...
                if (msg == EQUIP_ERR_OK)
                {
                    ...
                    ChatHandler(player->GetSession()).PSendSysMessage("Withdrew %u x %s.", stackSize, temp->Name1.c_str());
                }
                else
                {
                    player->SendEquipError(msg, nullptr, nullptr, entry);
                    ChatHandler(player->GetSession()).PSendSysMessage("Not enough bag space to withdraw %u x %s.", stackSize, temp->Name1.c_str());
                    return;
                }
            }
        }
    ...
    // Updates the item count maps and removes the item from the player's inventory
    ...
    void DepositAllReagents(Player *player)
    {
        ...
        if (entryToAmountMap.size() != 0)
        {
            ...
            CharacterDatabase.CommitTransaction(trans); // <-- just call, don't check return value
        }
        // Feedback to player
    ...
    {
        // Similar to DepositAllReagents, but only deposit items matching item_subclass
        WorldSession *session = player->GetSession();
        std::map<uint32, uint32> entryToAmountMap;
        std::map<uint32, uint32> entryToSubclassMap;
        std::map<uint32, uint32> itemsAddedMap;
        ...
        {
            ChatHandler(player->GetSession()).PSendSysMessage("No reagents to deposit in this category.");
        }
        CloseGossipMenuFor(player);
    }
    ...
    {
        QueryResult result = CharacterDatabase.Query(
            "SELECT item_entry, amount FROM mod_reagent_bank_account WHERE account_id = " +
            std::to_string(player->GetSession()->GetAccountId()) +
            " AND item_subclass = " + std::to_string(item_subclass));

        if (!result)
        {
            ChatHandler(player->GetSession()).PSendSysMessage("No reagents to withdraw in this category.");
            return;
        }
        ...
        do
        {
            ...
            while (remaining > 0)
            {
                ...
                ItemPosCountVec dest;
                InventoryResult msg = player->CanStoreNewItem(NULL_BAG, NULL_SLOT, dest, itemEntry, toGive);
                if (msg == EQUIP_ERR_OK)
                {
                    // Remove or update the reagent in the DB
                    if (toGive == remaining)
                        CharacterDatabase.Execute("DELETE FROM mod_reagent_bank_account WHERE account_id = {} AND item_entry = {}", player->GetSession()->GetAccountId(), itemEntry);
                    else
                        CharacterDatabase.Execute("UPDATE mod_reagent_bank_account SET amount = {} WHERE account_id = {} AND item_entry = {}", remaining - toGive, player->GetSession()->GetAccountId(), itemEntry);
                ...
                else
                {
                    ...
                    ChatHandler(player->GetSession()).PSendSysMessage("Not enough bag space to withdraw %u x %s.", toGive, temp->Name1.c_str());
                    break;
                }
            }
        } while (result->NextRow());
        ...
        if (!anyWithdrawn)
            ChatHandler(player->GetSession()).PSendSysMessage("No reagents withdrawn.");
    }
...
public:
    ...
    {
        g_maxOptionsPerPage = sConfigMgr->GetOption<uint32>("ReagentBankAccount.MaxOptionsPerPage", DEFAULT_MAX_OPTIONS);
    }
    ...
    bool OnGossipHello(Player *player, Creature *creature) override
    {
        ...
        AddGossipItemFor(player, GOSSIP_ICON_NONE, GetCachedItemIcon(2589, MAIN_ICON_SIZE, MAIN_ICON_SIZE, MAIN_ICON_X, MAIN_ICON_Y) + "Cloth", ITEM_SUBCLASS_CLOTH, 0);
        AddGossipItemFor(player, GOSSIP_ICON_NONE, GetCachedItemIcon(12208, MAIN_ICON_SIZE, MAIN_ICON_SIZE, MAIN_ICON_X, MAIN_ICON_Y) + "Meat", ITEM_SUBCLASS_MEAT, 0);
        AddGossipItemFor(player, GOSSIP_ICON_NONE, GetCachedItemIcon(2772, MAIN_ICON_SIZE, MAIN_ICON_SIZE, MAIN_ICON_X, MAIN_ICON_Y) + "Metal & Stone", ITEM_SUBCLASS_METAL_STONE, 0);
        AddGossipItemFor(player, GOSSIP_ICON_NONE, GetCachedItemIcon(10940, MAIN_ICON_SIZE, MAIN_ICON_SIZE, MAIN_ICON_X, MAIN_ICON_Y) + "Enchanting", ITEM_SUBCLASS_ENCHANTING, 0);
        AddGossipItemFor(player, GOSSIP_ICON_NONE, GetCachedItemIcon(7068, MAIN_ICON_SIZE, MAIN_ICON_SIZE, MAIN_ICON_X, MAIN_ICON_Y) + "Elemental", ITEM_SUBCLASS_ELEMENTAL, 0);
        AddGossipItemFor(player, GOSSIP_ICON_NONE, GetCachedItemIcon(4359, MAIN_ICON_SIZE, MAIN_ICON_SIZE, MAIN_ICON_X, MAIN_ICON_Y) + "Parts", ITEM_SUBCLASS_PARTS, 0);
        AddGossipItemFor(player, GOSSIP_ICON_NONE, GetCachedItemIcon(2604, MAIN_ICON_SIZE, MAIN_ICON_SIZE, MAIN_ICON_X, MAIN_ICON_Y) + "Other Trade Goods", ITEM_SUBCLASS_TRADE_GOODS_OTHER, 0);
        AddGossipItemFor(player, GOSSIP_ICON_NONE, GetCachedItemIcon(2453, MAIN_ICON_SIZE, MAIN_ICON_SIZE, MAIN_ICON_X, MAIN_ICON_Y) + "Herb", ITEM_SUBCLASS_HERB, 0);
        AddGossipItemFor(player, GOSSIP_ICON_NONE, GetCachedItemIcon(2318, MAIN_ICON_SIZE, MAIN_ICON_SIZE, MAIN_ICON_X, MAIN_ICON_Y) + "Leather", ITEM_SUBCLASS_LEATHER, 0);
        AddGossipItemFor(player, GOSSIP_ICON_NONE, GetCachedItemIcon(1206, MAIN_ICON_SIZE, MAIN_ICON_SIZE, MAIN_ICON_X, MAIN_ICON_Y) + "Jewelcrafting", ITEM_SUBCLASS_JEWELCRAFTING, 0);
        AddGossipItemFor(player, GOSSIP_ICON_NONE, GetCachedItemIcon(4358, MAIN_ICON_SIZE, MAIN_ICON_SIZE, MAIN_ICON_X, MAIN_ICON_Y) + "Explosives", ITEM_SUBCLASS_EXPLOSIVES, 0);
        AddGossipItemFor(player, GOSSIP_ICON_NONE, GetCachedItemIcon(4388, MAIN_ICON_SIZE, MAIN_ICON_SIZE, MAIN_ICON_X, MAIN_ICON_Y) + "Devices", ITEM_SUBCLASS_DEVICES, 0);
        AddGossipItemFor(player, GOSSIP_ICON_NONE, GetCachedItemIcon(23572, MAIN_ICON_SIZE, MAIN_ICON_SIZE, MAIN_ICON_X, MAIN_ICON_Y) + "Nether Material", ITEM_SUBCLASS_MATERIAL, 0);
        AddGossipItemFor(player, GOSSIP_ICON_NONE, GetCachedItemIcon(38682, MAIN_ICON_SIZE, MAIN_ICON_SIZE, MAIN_ICON_X, MAIN_ICON_Y) + "Armor Vellum", ITEM_SUBCLASS_ARMOR_ENCHANTMENT, 0);
        AddGossipItemFor(player, GOSSIP_ICON_NONE, GetCachedItemIcon(39349, MAIN_ICON_SIZE, MAIN_ICON_SIZE, MAIN_ICON_X, MAIN_ICON_Y) + "Weapon Vellum", ITEM_SUBCLASS_WEAPON_ENCHANTMENT, 0);
        ...
        else if (item_subclass == WITHDRAW_ALL_REAGENTS)
        {
            if (gossipPageNumber == 0)
            {
                ...
                std::vector<uint32> subclasses = {
                    ITEM_SUBCLASS_CLOTH, ITEM_SUBCLASS_MEAT, ITEM_SUBCLASS_METAL_STONE, ITEM_SUBCLASS_ENCHANTING,
                    ITEM_SUBCLASS_ELEMENTAL, ITEM_SUBCLASS_PARTS, ITEM_SUBCLASS_TRADE_GOODS_OTHER, ITEM_SUBCLASS_HERB,
                    ITEM_SUBCLASS_LEATHER, ITEM_SUBCLASS_JEWELCRAFTING, ITEM_SUBCLASS_EXPLOSIVES, ITEM_SUBCLASS_DEVICES,
    ...
    void ShowReagentItems(Player *player, Creature *creature, uint32 item_subclass, uint16 gossipPageNumber)
    {
        ...
        uint32 endValue = (gossipPageNumber + 1) * (g_maxOptionsPerPage) - 1;
        std::map<uint32, uint32> entryToAmountMap;
        ...
        AddGossipItemFor(player, GOSSIP_ICON_NONE,
            "|cff003366" + categoryName + ": " + std::to_string(totalItems) + " types, " + std::to_string(totalAmount) + " total|r",
            0, 0);

        // Deposit All button (bold green, consistent icon)
        AddGossipItemFor(player, GOSSIP_ICON_NONE,
            GetCachedItemIcon(2901, ICON_SIZE, ICON_SIZE, ICON_X, ICON_Y) + " |cff1eff00Deposit All|r",
            DEPOSIT_ALL_REAGENTS, item_subclass);

        // Withdraw All button (bold blue, consistent icon)
        AddGossipItemFor(player, GOSSIP_ICON_NONE,
            GetCachedItemIcon(2901, ICON_SIZE, ICON_SIZE, ICON_X, ICON_Y) + " |cff0070ddWithdraw All|r",
        ...
        {
            AddGossipItemFor(player, GOSSIP_ICON_NONE,
                GetCachedItemIcon(23705, ICON_SIZE, ICON_SIZE, ICON_X, ICON_Y) + " |cff003366Next Page|r ▶ (" +
        ...
        {
            AddGossipItemFor(player, GOSSIP_ICON_NONE,
                "◀ |cff003366Previous Page|r " + GetCachedItemIcon(23705, ICON_SIZE, ICON_SIZE, ICON_X, ICON_Y) +
                " (" + std::to_string(currentPage - 1) + "/" + std::to_string(totalPages) + ")",
                item_subclass, gossipPageNumber - 1);
        }

        // List items for this page with icon, colored link, and count in black
        for (uint32 i = startValue; i <= endValue; i++)
        {
            ...
            AddGossipItemFor(player, GOSSIP_ICON_NONE,
                icon + link + " |cff000000x " + std::to_string(amount) + "|r",
                itemEntry, gossipPageNumber);
        }

        // Back button to main menu (gray, consistent icon)
        AddGossipItemFor(player, GOSSIP_ICON_NONE,
            GetCachedItemIcon(6948, ICON_SIZE, ICON_SIZE, ICON_X, ICON_Y) + " |cff666666Back to Categories|r",
            MAIN_MENU, 0);
